...
# It uses 'feh' to display image previews.
# It requires Rofi, Oh My Posh, jq, feh, and a web browser configured with xdg-open (for docs).
...
if ! command -v feh &>/dev/null; then
  ...
  echo "Please install feh (e.g., sudo pacman -S feh) to display image previews."
  FEH_AVAILABLE=false
else
  FEH_AVAILABLE=true
fi
...
# Add the "Browse Previews" option to the list if jq and feh are available and the preview file exists
if [ "$JQ_AVAILABLE" = true ] && [ "$FEH_AVAILABLE" = true ] && [ -f "$PREVIEWS_JSON_FILE" ]; then
  theme_list="Browse Local Previews\n$theme_list"
elif [ "$JQ_AVAILABLE" = true ]; then
  # jq is available but feh or the preview file is not
...
"Browse Local Previews")
  ...
  if [ "$FEH_AVAILABLE" = false ]; then
    echo "Cannot browse previews: feh is not available."
    exit 1
  fi
  ...
  # Open the selected image URL using feh
  echo "Opening image with feh: $selected_image_url"
  feh "$selected_image_url" & # Use '&' to run feh in the background so Rofi closes
